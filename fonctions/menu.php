...
class Menu{
	...
	public function creerMenuListe($composantActif, $nomMenu, $uneLigue){
	    $menu = "<select class = '" .  $this->style . "' onchange='window.location.href=this.value'>";
	    $menu .= "<option>";
	    $menu .= "-- Sélectionnez un club --";
	    $menu .= "</option>";
	    foreach($this->composants as $composant){
	        if($composant[0] == $composantActif){
	            $menu .= "<option value='?ligue=".$uneLigue->getIdLigue()."&club=". $composant[0]."'>";
	            $menu .= $composant[1];
	        }else{
	            $menu .= "<option value='?ligue=".$uneLigue->getIdLigue()."&club=". $composant[0]."'>";
	            $menu .= $composant[1];
